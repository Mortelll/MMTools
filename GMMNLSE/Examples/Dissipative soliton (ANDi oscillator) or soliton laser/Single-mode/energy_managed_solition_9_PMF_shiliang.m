...
gain_rate_eqn.copump_power = 0.300; % W 泵浦功率
...
fiber_ND.L0 = 0.9;
fiber_ND.betas = [5.8126e6, 5.8126e6;   % β0 
                  4.89e3,   4.88e3;     % β1 
                 -22.8e-3, -22.5e-3]';  % β2 
...
fiber_Gain.L0 = 1.2;
fiber_Gain.MFD = 9.5;
fiber_Gain.betas = [5.8126e6, 5.8126e6;   % β0
                    5.0e3,    4.98e3;     % β1
                   -25.5e-3, -25.0e-3]';  % β2
...
prop_output = build_MMgaussian(tfwhm, time_window, total_energy,2,Nt);
...
field = cell(1,max_rt); 
splice_z = cumsum([fiber_ND.L0, SF_length, SA_length, SF_length, fiber_Gain.L0, Ps_length, fiber_ND1.L0, fiber_ND1.L0, Ps_length,fiber_Gain.L0,]);% 接合点的位置
...
while rt_num < max_rt
    ...
    % -----------------------------------------------------------------
    % % 输出耦合器
    % E_in_from_cavity = prop_output.fields(:,:,end);
    ...
    % end
    % prop_output1 = struct();
    % prop_output1.fields = Outputs(:,:,1); % 顺时针循环脉冲
    % prop_output1.dt = prop_output.dt;     % 保持相同的时间步长
    % 
    % prop_output2 = struct();
    % prop_output2.fields = Outputs(:,:,2); % 逆时针循环脉冲
    % prop_output2.dt = prop_output.dt;     % 保持相同的时间步长

    % 输出耦合器
    E_in_from_cavity = prop_output.fields(:,:,end); % size: [Nt, 2] for two polarizations
    E_in_from_other_port = zeros(size(E_in_from_cavity)); % size: [Nt, 2]
    Outputs = zeros(Nt, 2, 2); % [Nt, polarization, port]
    Coupler_Matrix = [sqrt(1-OC),      1i*sqrt(OC); ...
                      1i*sqrt(OC),      sqrt(1-OC)];
    for pol = 1:2
        Inputs_pol = [E_in_from_cavity(:, pol), E_in_from_other_port(:, pol)]; % [Nt, 2]
        Outputs_pol = zeros(Nt, 2);
        for i = 1:Nt
            input_vector = Inputs_pol(i, :).'; % [2,1]
            output_vector = Coupler_Matrix * input_vector;
            Outputs_pol(i, :) = output_vector.';
        end
        Outputs(:, pol, :) = reshape(Outputs_pol, [Nt, 1, 2]);
    end
    prop_output1.fields = Outputs(:,:,1); % [Nt, 2] for port 1
    prop_output2.fields = Outputs(:,:,2); % [Nt, 2] for port 2
    ...
    % 相移器
    beta_PS = -pi/2;
    prop_output1.fields = prop_output1.fields*exp(1i*beta_PS);
    [saved_field,saved_z_Ps] = func.extract_saved_field(prop_output.fields, 1, current_z, Ps_length);

    field{rt_num}(:,:,zn) = saved_field;
    saved_z(zn) = saved_z_Ps; 
    ...
    % 相移器
    beta_PS = -pi/2;
    prop_output2.fields = prop_output2.fields*exp(1i*beta_PS);
    [saved_field,saved_z_Ps] = func.extract_saved_field(prop_output.fields, 1, current_z, Ps_length);
    ...
    % -----------------------------------------------------------------
    % % 输出耦合器
    % E_in_from_cavity = prop_output1.fields(:,:,end);
    % E_in_from_other_port = prop_output2.fields(:,:,end);
    % Inputs = cat(3, E_in_from_cavity, E_in_from_other_port);
    % Coupler_Matrix = [sqrt(1-OC),      1i*sqrt(OC); ...
    %                  1i*sqrt(OC),      sqrt(1-OC)];
    % Outputs = zeros(size(Inputs)); % Preallocate output, size [Nt, 1, 2]
    ...
    %     Outputs(i, 1, 2) = output_vector(2);
    % end
    % prop_output.fields = Outputs(:,:,1); % 反馈回主腔的脉冲
    % output_field(:,:,rt_num) = Outputs(:,:,2);    % 即将输出的脉冲

    % 输出耦合器
    E_in_from_cavity = prop_output1.fields(:,:,end); % [Nt, 2] 
    E_in_from_other_port = prop_output2.fields(:,:,end); % [Nt, 2]
    Outputs = zeros(Nt, 2, 2); % [Nt, polarization, port]
    Coupler_Matrix = [sqrt(1-OC),      1i*sqrt(OC); ...
                      1i*sqrt(OC),      sqrt(1-OC)];
    for pol = 1:2
    ...
    prop_output.fields = Outputs(:,:,1); % [Nt, 2] 
    output_field(:,:,rt_num) = Outputs(:,:,2); % [Nt, 2]
    ...
    % -----------------------------------------------------------------
    % 输出场的能量
    output_energy(rt_num) = trapz(sum(abs(output_field(:,:,rt_num)).^2, 2))*prop_output.dt/1e3; % 能量（nj)
