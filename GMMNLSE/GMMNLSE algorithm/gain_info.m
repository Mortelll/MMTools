...
if isequal(sim.step_method,'RK4IP') % single spatial mode
    ...
    % ====== 如果 core_region 未定义,创建一个简单的 ======
    if ~exist('core_region', 'var')
        % 对于"伪多模"(如双波长)情况,创建统一的核心区域
        Nx = 50;  % 简单网格
        x_range = linspace(-gain_rate_eqn.core_diameter, gain_rate_eqn.core_diameter, Nx);
        [x_grid, y_grid] = meshgrid(x_range, x_range);
        core_region = (x_grid.^2 + y_grid.^2) <= (gain_rate_eqn.core_diameter/2)^2;
        
        % 保存 dx 用于后续积分
        gain_rate_eqn.mode_profile_dx = abs(x_range(2) - x_range(1));
    end
    % ===================================================
