...
func.analyze_fields1     = @analyze_fields1;
...
function fig = analyze_fields1(t,f,fields,saved_z,splice_z,fig_title_prefix)
% ANALYZE_FIELDS_WITHIN_CAVITY1 This function generates two plots of the
% information of the fields along the fibers.
% If the number of modes considered here is larger than one, midx>1, it'll
% plot only the total energy along the fibers; otherwise, it'll plot two
% figures, one with energy, bandwidth, and duration, the other with two
...
%   t: (N,1); time (ps)
%   f: (N,1); frequency (THz)
%   fields: (N,num_components,num_fields); 
%           Input the components you want to combine (e.g. [Ex, Ey])
%   saved_z: (1,num_saved_z); z position
%   splice_z: (1,num_splice); splice positions
%   fig_title_prefix: string; prefix for figure titles (e.g. 'Lambda1_Total')
%
% Output:
%   fig: handle of the generated figures
%
% -------------------------------------------------------------------------
% Unit explanation:
%   intensity = abs(field).^2;
...
if ~exist('fig_title_prefix','var')
    fig_title_prefix = 'Total Field';
end
...
wavelength = c./f; % nm
positive_wavelength = wavelength>0; % exclude those negative wavelengths due to a large frequency window
intensity_components = abs(fields).^2; % W
total_intensity = sum(intensity_components, 2); 
%peak_power = permute(max(intensity),[3 2 1]);
% energy = permute(trapz(intensity),[3 2 1])*dt/1e3; % nJ
factor_correct_unit = (Nt*dt)^2/1e3; % to make the spectrum of the correct unit "nJ/THz"
                                     % "/1e3" is to make pJ into nJ
spectrum_components = abs(fftshift(ifft(fields,[],1),1)).^2*factor_correct_unit; % in frequency domain
total_spectrum = sum(spectrum_components, 2);
total_pulse_energy = permute(trapz(total_intensity, 1),[3 2 1])*dt/1e3;
%1.  "real" is just to suppress the warning if calc_RMS gives a warning about
%    its imaginary output.
%    If the input is really noisy, the calculated RMS can be imaginary.
% 2. Cleaning the background noise is important for RMS calculations.
intensity_clean_background = total_intensity; 
spectrum_clean_background = total_spectrum;


for zidx = 1:size(fields,3)
    intensity_clean_background(total_intensity(:,1,zidx)<max(total_intensity(:,1,zidx))*1e-3,1,zidx) = 0;
    spectrum_clean_background(total_spectrum(:,1,zidx)<max(total_spectrum(:,1,zidx))*1e-3,1,zidx) = 0;
end
...
factor = c./wavelength(positive_wavelength).^2; % change the spectrum from frequency domain into wavelength domain
spectrum_lambda = total_spectrum(positive_wavelength, :, :) .* factor;
%{
% Calculate the Strehl ratio
grating_incident_angle = pi/6;
grating_spacing = 1e-6; % assume a 1000 line/mm grating here
...
% Figure 1: Detail Info (Energy, Bandwidth, Duration)
fig(1) = figure('Name', [fig_title_prefix ' - Detail']);
fp = get(fig(1),'position');
screen_size = get(0,'ScreenSize');
original_top = screen_size(4)-fp(2)-fp(4);
set(fig(1),'position',[fp(1) screen_size(4)-original_top-fp(4)*7/4 fp(3)*5/4 fp(4)*7/4]);
...
% Subplot 1: Energy
subplot(3,1,1); 
h = plot(saved_z, total_pulse_energy); 
xlim([saved_z(1) saved_z(end)]);
xlabel('Propagation length (m)'); ylabel('Pulse Energy (nJ)');
title(['Energy: ' fig_title_prefix]);
set(h,'linewidth',2); set(gca,'fontsize',14);
grid on;
hold on;
% Draw Splice Lines
if ~isempty(splice_z)
    for i = 1:length(splice_z)
        h = plot([splice_z(i);splice_z(i)], [min(total_pulse_energy(:)); max(total_pulse_energy(:))]);
        set(h,'linewidth',2,'linestyle','--','color','black','marker','none');
    end
end
hold off;
...
subplot(3,1,[2 3]);
yyaxis left;
h = plot(saved_z, bandwidth); 
xlim([saved_z(1) saved_z(end)]);
xlabel('Propagation length (m)'); ylabel('Bandwidth (nm)');
title(['RMS Info: ' fig_title_prefix]);
set(h,'linewidth',2); set(gca,'fontsize',14);
hold on;
% Draw Splice Lines (Bandwidth) 
if ~isempty(splice_z)
    for i = 1:length(splice_z)
        y_min = min(bandwidth);
        y_max = max(bandwidth);
        if isnan(y_min), y_min=0; end
        if isnan(y_max) || y_max==y_min, y_max=y_min+1; end
        h = plot([splice_z(i);splice_z(i)], [y_min; y_max]);
        set(h,'linewidth',2,'linestyle','--','color','black','marker','none');
    end
end
hold off;
yyaxis right;
h = plot(saved_z, duration); 
xlim([saved_z(1) saved_z(end)]);
ylabel('Duration (ps)');
set(h,'linewidth',2); set(gca,'fontsize',14);

% % Draw Splice Lines for RMS plot
% if ~isempty(splice_z)
%     ylim_left = get(gca, 'YLim'); 
%     % Note: Drawing vertical lines on dual-axis plot can be tricky, keeping it simple here.
% end

% Figure 2: Evolution (Colormaps)
fig(2) = figure('Name', [fig_title_prefix ' - Evolution']);
fp = get(fig(2),'position');
set(fig(2),'position',[fp(1) screen_size(4)-original_top-fp(4)*7/4 fp(3)*5/4 fp(4)*7/4]);

% Subplot 1: Time Domain Evolution
subplot(2,1,1); 
pcolor(saved_z, t, permute(total_intensity, [1 3 2])); 
shading interp; cb = colorbar; 
ylim([min(t) max(t)]);
xlabel('Propagation length (m)'); ylabel('Time (ps)');
title(['Pulse Evolution: ' fig_title_prefix]);
set(cb,'Location','southoutside');
set(gca,'fontsize',14);
% Add Splice Lines
if ~isempty(splice_z)
    hold on;
    for i = 1:length(splice_z)
        plot(splice_z(i)*[1 1], [min(t) max(t)], 'w--', 'LineWidth', 2);
    end
    hold off;
end

% Subplot 2: Wavelength Domain Evolution
subplot(2,1,2); 
pcolor(saved_z, wavelength(positive_wavelength), permute(10*log10(spectrum_lambda + eps), [1 3 2])); 
shading interp; cb = colorbar; 
ylim([min(wavelength(positive_wavelength)) max(wavelength(positive_wavelength))]);
xlabel('Propagation length (m)'); ylabel('Wavelength (nm)');
title(['Spectrum Evolution: ' fig_title_prefix]);
set(cb,'Location','southoutside');
set(gca,'fontsize',14);
% Add Splice Lines
if ~isempty(splice_z)
    hold on;
    w_lims = [min(wavelength(positive_wavelength)) max(wavelength(positive_wavelength))];
    for i = 1:length(splice_z)
        plot(splice_z(i)*[1 1], w_lims, 'w--', 'LineWidth', 2);
    end
    hold off;
end
